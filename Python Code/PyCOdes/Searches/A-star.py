    ...
    while len(queue) > 0: 
        ...
        if state[0] < capacity1: #Fill Jug A 
            ...
            if next_state not in visited:
                N_node = (next_state,Utility(next_state,state,goal_state))
...
def Utility(nState, pState, goal_state):
    new = (nState[0] + nState[1])
    old = (pState[0] + pState[1])
    dis = (goal_state[0] - nState[0])**2 + (goal_state[1] - nState[1])**2
    if new > old: return new + dis
    return old + dis
